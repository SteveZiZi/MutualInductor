...
namespace XT{
	...
	public:
		PolyFit():ssr(0),sse(0),rmse(0){factor.resize(2,0);}
		~PolyFit(){}
		///
		...
		///
		template<typename T>
                bool linearFit(const T& x, const T& y,bool isSaveFitYs=false)
		{
		...
		///
		template<typename T>
                size_t getSeriesLength(const T& x
                        ,const T& y)
		{
